...
function App() {
  ...
  const getPopulationByPref = (pref) => {
    axios.get(`population/composition/perYear?cityCode=-&prefCode=${pref.prefCode}`)
      .then(response => {
        const population = response.data.result.data;
        const totalPopulation = population.find(item => item.label === "総人口").data;
  ...
  const addChartData = (pref, popPerYear) => {
    ...
    for (let i = 0; i < popPerYear.length; i++) {
      data[i] = {
        ...data[i],
  ...
  const prefLines = prefectures.filter(pref => pref.selected).map(pref => (
    <Line type="monotone" dataKey={pref.prefName} stroke={`#${Math.floor(Math.random() * 16777215).toString(16)}`} />
  ))

  const renderLineChart = (
    <LineChart width={600} height={400} data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: 20 }}>
      {prefLines}
